...
describe("Header", () => {
  ...
  it('should render logoutSection with isLoggedIn true in context', () => {
      const wrapper = shallow(<Header />, { context: { user: { isLoggedIn: true } } });
      expect(wrapper.find('#logoutSection')).toHaveLength(1);
  });

  it('should call logOut function when logout link is clicked', () => {
      const logOutSpy = jest.fn();
      const wrapper = shallow(<Header />, { context: { user: { isLoggedIn: true, email: 'test@example.com' }, logOut: logOutSpy } });
