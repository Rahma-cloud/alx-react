...
test('returns expected data for a specific user', () => {
  const expectedData = [
    {
...
test('verifies the correct result array', () => {
  ...
  expect(Object.keys(normalizedNotifications.entities.notifications)).toEqual(expect.arrayContaining(expectedResults));
});

test('verifies the correct users entity', () => {
  const expectedUser = {
    age: 25,
...
test('verifies the correct messages entity', () => {
  ...
  expect(normalizedNotifications.entities.messages[messageId]).toEqual(expectedMessage);
});

test('verifies the correct notifications entity', () => {
